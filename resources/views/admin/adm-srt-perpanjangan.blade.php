...
@endpush
@section('page')
  <li class="breadcrumb-item active">Surat Tugas Perpanjangan</li>
@endsection
@section('content')
<div class="col-md-12">
    <div class="card">
        ...
        <div class="card-body">
          <table class="table table-bordered table-striped table-hover">
            ...
            <tbody>
                @foreach ( $data_perpanjang as $key => $data )
                <tr>
                    <tr>
                        <td>{{ $key + 1 }}</td>
                    ...
                    <td align="center">
                        @if($data->status == 'diperpanjang_kaprodi')
                        <button class="btn btn-success btn-block btn-sm" data-toggle="modal" data-target="#setujui-{{ $key }}" >Setujui</button>
                        @else
                        <a href="{{ route('SuratTugas.cancelPerpanjang', ['id' => $data->id_surat_tugas]) }}" class="btn btn-danger btn-block btn-sm" >Cancel</a>
                        @endif
                    </td>
                    <td align="center">
        ...
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <table class="col-lg">
                        <tr align="center">
                            <td>Surat Tugas Sebelumnya</td>
                            <td>Proposal Skripsi</td>
                        </tr>
                        <tr>
                            <td><iframe src="{{ asset($data->suratSebelumnya()->first()->url_surat_tugas) }}" title="description" style="height: 350px;width:350px;"></iframe></td>
                            <td><iframe src="{{ asset($data->url_bab1) }}" title="description" style="height: 350px;width:350px;"></iframe></td>
                        </tr>
                    </table>
                </div>
            </div>
...
<form action="{{ route('SuratTugas.updatePerpanjang', ['id' => $data->id_surat_tugas]) }}" method="POST" enctype="multipart/form-data">
    @csrf
    <!-- Extra large modal -->
    <div class="modal fade bd-example-modal-md" id="setujui-{{ $key }}" tabindex="-1" role="dialog" aria-labelledby="myExtraLargeModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title">Kirim Surat Tugas</h4>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <div class="custom-file">
                        <input name="url_surat_tugas" type="file" class="custom-file-input" id="exampleInputFile1">
