...
@endpush
@section('page')
  <li class="breadcrumb-item active">Pengajuan Revisi Seminar Proposal</li>
@endsection
@section('content')
<div class="col-md-12">
    <div class="card">
        ...
        <div class="card-body">
          <table class="table table-bordered table-striped table-hover">
            ...
            <tbody>
                @foreach ( $data_rvs_sempro as $key => $data )
                <tr>
                    <td>{{ $key + 1 }}</td>
                    ...
                    <td align="center">
                        @if ($data->status_surat_skripsi == 'belum_approve')
                        <button class="btn btn-success btn-block btn-sm" data-toggle="modal" data-target="#setujui-{{ $key }}" >Setujui</button>
                        @else
                        <a href="{{ route('MhsBimbingan.cancelRvsSempro', ['id' => $data->id_mhs_bimbingan]) }}" class="btn btn-danger btn-block btn-sm" >Batal</a>
                        @endif
                    </td>
                    {{-- <td align="center">
...
<form action="{{ route('MhsBimbingan.updateDosenRvsSempro', ['id' => $data->id_surat_tugas]) }}" method="POST" enctype="multipart/form-data">
    @csrf
    <!-- Extra large modal -->
