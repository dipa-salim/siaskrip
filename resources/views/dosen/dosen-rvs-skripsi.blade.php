...
@endpush
@section('page')
  <li class="breadcrumb-item active">Pengajuan Revisi Sidang Skripsi</li>
@endsection
@section('content')
<div class="col-md-12">
    <div class="card">
        ...
        <div class="card-body">
          <table class="table table-bordered table-striped table-hover">
            ...
            <tbody>
                @foreach ( $data_rvs_skripsi as $key => $data )
                <tr>
                    <td>{{ $key + 1 }}</td>
                    ...
                    <td align="center">
                        @if ($data->status_hasil_skripsi == 'belum_approve')
                        <button class="btn btn-success btn-block btn-sm" data-toggle="modal" data-target="#setujui-{{ $key }}" >Setujui</button>
                        @else
                        <a href="{{ route('MhsBimbingan.cancelRvsSkripsi', ['id' => $data->id_mhs_bimbingan]) }}" class="btn btn-danger btn-block btn-sm" >Batal</a>
                        @endif
                    </td>
                    {{-- <td align="center">
...
<form action="{{ route('MhsBimbingan.updateDosenRvsSkripsi', ['id' => $data->id_mhs_bimbingan]) }}" method="POST" enctype="multipart/form-data">
    @csrf
    <!-- Extra large modal -->
    <div class="modal fade bd-example-modal-md" id="setujui-{{ $key }}" tabindex="-1" role="dialog" aria-labelledby="myExtraLargeModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title">Kirim Persetujuan Revisi Skripsi</h4>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                    </button>
                </div>
