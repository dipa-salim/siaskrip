    ...
    <div class="card">
        ...
        <div class="card-body">
          <table class="table table-bordered table-striped table-hover">
            ...
            <tbody>
                @foreach ( $data_pengajuan as $key => $data )
                <tr>
                    <td>{{ $key + 1 }}</td>
                    ...
                    <td align="center">
                        @if ($data->status_surat_tugas == 'belum_approve')
                        <button class="btn btn-success btn-block btn-sm" data-toggle="modal" data-target="#setujui-{{ $key }}" >Setujui</button>
                        @else
                        <a href="{{ route('MhsBimbingan.cancelSurat', ['id' => $data->id_mhs_bimbingan]) }}" class="btn btn-danger btn-block btn-sm" >Batal</a>
                        @endif
                    </td>
                    {{-- <td align="center">
...
<form action="{{ route('MhsBimbingan.update', ['id' => $data->id_mhs_bimbingan]) }}" method="POST" enctype="multipart/form-data">
    @csrf
    <!-- Extra large modal -->
    <div class="modal fade bd-example-modal-md" id="setujui-{{ $key }}" tabindex="-1" role="dialog" aria-labelledby="myExtraLargeModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title">Kirim Lembar Persetujuan Dosen Pembimbing</h4>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <div class="form-group">
                        <label for="exampleInputFile1">Surat Persetujuan Dosen (beri tanda-tangan)</label>
                        <div class="input-group">
                          <div class="custom-file">
                            <input name="url_surat_dosen_ttd" value="{{ old('url_surat_dosen_ttd') }}" type="file" class="custom-file-input" id="exampleInputFile1">
