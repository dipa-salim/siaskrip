    ...
    <div class="card">
        ...
        <div class="card-body">
          <table class="table table-bordered table-striped table-hover">
            ...
            <tbody>
                @foreach ( $data_skripsi_kaprodi as $key => $data )
                <tr>
                    <td>{{ $key + 1 }}</td>
                    ...
                    <td align="center">
                        @if ($data->status == 'belum_approve_kaprodi')
                        <button class="btn btn-success btn-block btn-sm" data-toggle="modal" data-target="#setujui-{{ $key }}" >Setujui</button>
                        @else
                        <a href="{{ route('Skripsi.cancelKaprodi', ['id' => $data->id_skripsi]) }}" class="btn btn-danger btn-block btn-sm" >Batal</a>
                        @endif
                    </td>
                </tr>
                @endforeach
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <table class="col-lg">
                        ...
                        <tr>
                        <td><iframe src="{{ asset($data->url_biodata) }}" title="description" style="height: 350px;width:350px;"></iframe></td>
                        <td><iframe src="{{ asset($data->url_pkl) }}" title="description" style="height: 350px;width:350px;"></iframe></td>
                        </tr>
                        <tr align="center">
                            <td>Bukti PKM</td>
                            <td>Scan KTM</td>
                        ...
                        <tr>
                        <td><iframe src="{{ asset($data->url_pkm) }}" title="description" style="height: 350px;width:350px;"></iframe></td>
                        <td><iframe src="{{ asset($data->url_ktm) }}" title="description" style="height: 350px;width:350px;"></iframe></td>
                        </tr>
                        <tr align="center">
                            <td>Materi Proposal</td>
                        </tr>
                        <tr>
                        <td><iframe src="{{ asset($data->url_skripsi) }}" title="description" style="height: 350px;width:350px;"></iframe></td>
                        </tr>
                    </table>
                </div>
            </div>
...
<form action="{{ route('Skripsi.updateKaprodi', ['id' => $data->id_skripsi]) }}" method="POST" enctype="multipart/form-data">
    @csrf
    <!-- Extra large modal -->
    <div class="modal fade bd-example-modal-md" id="setujui-{{ $key }}" tabindex="-1" role="dialog" aria-labelledby="myExtraLargeModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title">Kirim Jadwal Skripsi</h4>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                    ...
                    <div class="form-group">
                     ...
                     <div class="form-group">
                          <label for="dosen_penguji1">Dosen Penguji 1</label>
                            {{-- <input type="text" class="form-control" id="dosen_pembimbing1" name="dosen_pembimbing1" value="{{ old('dosen_pembimbing1') }}" type="text" class="form-control" rows='2' placeholder="Masukkan Judul" old> --}}
                            <select class="form-control" id="dosen_penguji1" name="dosen_penguji1">
                                <option value="">-- Select --</option>
                            @foreach ($dosen as $data)
                            <option value="{{ $data->id_dosen }}">{{ $data->nama }}</option>
                            @endforeach
                          ...
                          <input id="dosen_penguji1_text" type="text" placeholder="Nama Dosen Pembimbing" class="form-control" name="dosen_penguji1_text">
                          </div>
                          <div class="form-group">
                            <label for="dosen_penguji2">Dosen Penguji 2</label>
                            {{-- <input type="text" class="form-control" id="dosen_pembimbing2" name="dosen_pembimbing2" value="{{ old('dosen_pembimbing2') }}" type="text" class="form-control" rows='2' placeholder="Masukkan Judul" old> --}}
                            <select class="form-control" id="dosen_penguji2" name="dosen_penguji2">
                                <option value="">-- Select --</option>
                            @foreach ($dosen as $data)
                            <option value="{{ $data->id_dosen }}">{{ $data->nama }}</option>
                            @endforeach
                          ...
                          <input id="dosen_penguji2_text" type="text" placeholder="Nama Dosen Pembimbing" class="form-control" name="dosen_penguji2_text">
                          </div>
                </div>
...
<script>
    $(document).ready(function(){
        ...
        $("#dosen_penguji1_text").hide();
        $("#dosen_penguji2_text").hide();
        ...
        $('#dosen_penguji1').on('change', function(){
            let kel = $(this).val();
            if(kel == 0) {
                $("#dosen_penguji1_text").show();
            } else {
                $("#dosen_penguji1_text").hide();
            }
        });

        $('#dosen_penguji2').on('change', function(){
            let kel = $(this).val();
            if(kel == 0) {
                $("#dosen_penguji2_text").show();
            } else {
                $("#dosen_penguji2_text").hide();
            }
        });
