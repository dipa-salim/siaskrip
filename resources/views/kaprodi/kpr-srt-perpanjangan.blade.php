    ...
    <div class="card">
        ...
        <div class="card-body">
          <table class="table table-bordered table-striped table-hover">
            ...
            <tbody>
                @foreach ( $data_perpanjang_kaprodi as $key => $data )
                <tr>
                    <td>{{ $key + 1 }}</td>
                    ...
                    <td align="center">
                        @if($data->status == 'belum_diperpanjang_kaprodi')
                        <button class="btn btn-success btn-block btn-sm" data-toggle="modal" data-target="#setujui-{{ $key }}" >Setujui</button>
                        @else
                        <a href="{{ route('SuratTugas.cancelKaprodiPerpanjang', ['id' => $data->id_surat_tugas]) }}" class="btn btn-danger btn-block btn-sm" >Cancel</a>
                        @endif
                    </td>
                </tr>
                @endforeach
...
<form action="{{ route('SuratTugas.updateKaprodiPerpanjang', ['id' => $data->id_surat_tugas]) }}" method="POST" enctype="multipart/form-data">
    @csrf
    <!-- Extra large modal -->
    <div class="modal fade bd-example-modal-md" id="setujui-{{ $key }}" tabindex="-1" role="dialog" aria-labelledby="myExtraLargeModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title">Kirim Surat Tugas Perpanjangan</h4>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                    </button>
                </div>
